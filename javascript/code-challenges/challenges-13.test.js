...
const longestString = (arr) => {
  let newLonStr = { string: '', index: -1 };
  arr.forEach((str, index) => {
    str.length > newLonStr.string.length ? newLonStr = { string: str, index: index } : '';

  });

  return newLonStr.index;
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(num => {
    let fir = num.substring(1, 4);
    let mid = num.substring(6, 9);
    let las = num.substring(10);
    return fir + mid + las;
...
const onlyOddChars = (str) => {
  return str.split('').filter((e, idx) => idx % 2 === 1) .join('');
};
